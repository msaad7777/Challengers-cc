...
### Content lives in `data.ts` files, not a CMS

There is no CMS and no content collection in Firestore. Every piece of editorial/reference content is a typed array in a sibling `data.ts`, edited in-repo and shipped on deploy:
...
| `app/c3h/pavilion/governanceDocs.ts` | `/c3h/pavilion` | `GovernanceDoc[]` |

Fixtures are the exception that has no single home — see the duplication warning under *Known duplication and gotchas*.
...
- `/c3h/events` — club calendar from the in-repo `events` array in `app/c3h/events/data.ts` (no Firestore). That file also holds the pure calendar helpers — `getUpcomingEvents` / `getPastEvents`, `googleCalendarUrl`, `generateICS` / `generateAllEventsICS` (the "subscribe to all" .ics), `formatEventDate`, `matchDetailsToEvent` — plus RRULE handling for recurring entries. Currently one recurring event (Wednesday training); league fixtures are **not** here.
- `/c3h/watch`, `/c3h/profile`, `/c3h/field-editor`
...
**Scorer domain types** — `app/c3h/scorer/types.ts` is the canonical shape for a scored match (`BallEvent`, `Innings`, `Match`) plus the pure derivations `createEmptyInnings` / `getBattingStats` / `getBowlingStats`. Everything downstream (`app/c3h/lib/matchStats.ts`, `MatchSummary`, `/c3h/live`, `playerAnalysis`) reads that shape, and `tests/factories.ts` builds fixtures against it — so a change here ripples through the whole scoring/analysis chain. Covered by `tests/scorerTypes.test.ts`.
...
- **`scripts/`** holds Firestore one-off maintenance scripts (e.g. `clean-lpl-roles.mjs`) that re-init Firebase with the same hardcoded config from `lib/firebase.ts`. They're meant to be run manually with `node scripts/<name>.mjs`, not part of the build. ⚠️ The directory is **untracked** (`?? scripts/` in `git status`) and not gitignored — it exists only on the local machine, so don't assume a script referenced in history is present, and don't blind-stage `scripts/`.
...
**Static assets**: `/public/...` (images, videos), `/public/documents/` (PDFs). Reference with leading slash (`/image.jpg`). ⚠️ **Everything under `/public/` is served publicly and unauthenticated** — `challengerscc.ca/documents/<filename>` needs no login and is crawlable. The root-level `.gitignore` guards (`/*.pdf`, `/*.jpeg`, …) deliberately **do not** apply inside `public/`, so a grant application, ID scan, or bank letter dropped there will be committed and published. Check what is in `public/documents/` before committing.
...
**Change the match schedule — two files, both required**: `app/schedule/page.tsx` (public: `lclT30Matches` / `lplT30Matches` / `lclT20Matches`) **and** `ALL_MATCHES` near the top of `app/c3h/availability/page.tsx` (Dugout). Editing one leaves the site self-contradictory with no build or test failure.
