...
export default function WatchPage() {
  ...
  useEffect(() => {
    if (status === 'unauthenticated') {
      router.push('/c3h/login?callbackUrl=/c3h/watch');
    }
  }, [status, router]);
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-10">
            <div className="inline-flex items-center gap-2 bg-red-500/20 px-4 py-2 rounded-full mb-4">
              ...
              <span className="text-sm font-semibold text-red-400">C3H · Match Day Live Stream</span>
            </div>
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold mb-4">
              Watch <span className="gradient-text">Live</span>
            </h1>
            <p className="text-gray-300 text-lg max-w-2xl mx-auto">
              Welcome, {session.user?.name?.split(' ')[0] || 'Challenger'}. Tune in to Challengers
              CC matches live. When we&apos;re streaming, the match appears below. Otherwise, catch
              up on past matches and highlights from our archive.
            </p>
          </div>
